...
<center>
    <ul class="nav nav-tabs nav-justified" id="strategy">
        <c:forEach items="${sessionScope.faceMasks}" var="i">
            <li role="presentation">
                <a href="guide?action=queryAllMaskProbByFMethod&fm_method=${i.fm_method}" onclick="showNav()">${i.fm_method}</a>
            </li>
        </c:forEach>
    </ul>
</center>
    ...
    <div class="col-md-12">
        <table class="table table-hover" style="vertical-align: center">
            <tr>
                ...
                <th><h3>医生回答</h3></th>
                <th><h3>医生信息</h3></th>
                <th><h3>所属口罩使用方法</h3></th>
            </tr>
            <c:forEach items="${requestScope.page.items}" var="i" varStatus="s">
                <tr>
                    <td><h3>${i.fp_question}</h3></td>
                    <td><h3>${i.fp_answer}</h3></td>
                    <td><h3>${i.d_doctorInfo}</h3></td>
                    <td><h3>${i.fm_method}</h3></td>
                </tr>
            </c:forEach>
